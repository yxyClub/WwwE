...
> 20180815上午索说下午2点有个重要会议要开，大家分别仔细考虑一下自己负责的那块工作的工作流程。下面，我先写个初稿。参考了一些我以前写的文章。
...
- 温盐实验预备：将待校仪器在实验室调试好。
> 具体包括：线、接口、软件匹配-直读式的调节到可以直接读取数据-自容式的调节到至时间后自动记录数据、建立相关文件夹存储实验数据信息、转移到海水中。
...
2.清洗仪器——当天做完仪器最好当天捞出来并用自来水冲洗；
3.转移仪器——将做完温盐实验的设备转移到压力实验室（如果需要）。
> 如果上述工作在第一天没做的话，这需要第二天早上来进行，如果完成的顺利的话，这一步就省略了。
4.将“标准温度记录表”复印若干份放到公共的文件夹下，将其原件放到穆的文件夹下。
...
- A.直接查看未完结文件夹中的委托单，看是否有只做压力项目的仪器，有的话，拍摄委托单，找到这些仪器拿到压力实验室等待实验。
- B.看压力试验室内摆放了哪些仪器（一般是做完了温盐实验后的），根据这些仪器的编号去查委托单，拍摄委托单。根据委托单上的单位名称、仪器型号和编号等安排实验。
- 上述完成后，要进行查验，仪器是否做压力实验，如果不能，则打电话与客户沟通。如果能，则继续安排下一步事宜。
- 鉴于客户在委托单上往往不写压力的量程，这里有两种方式来获得量程数据：一是搜索仪器的出厂编号，看看以往该仪器的量程是多少；第二种，如果上述信息不存在，则根据委托单上的联系方式，打电话给客户，询问量程。
...
- 1. 根据客户的紧急情况安排顺序。
- 2. 根据仪器的量程（一般同一量程范围的仪器一起做，节省换活塞和换表格）
- 3. 压力接口的特殊性。如果需要更换量程或者压力接口的，则优先级往后排。

### 实验准备：
- 根据量程，打印相关量程和标准器对应的表格。
- 建立实验当天的相关文件夹（命名方式举例：20180815海洋二所SBE37IM-5台p--下面根据各台自身的编号分别建立单独的文件夹）。
- 至少提前半小时，将恒温恒湿实验室打开，使温湿度达到实验要求，并使仪器达到环境稳定状态。
- 安装好相应的接口，准备连接标准压力装置，使用连接件连接好仪器的压力接口
- 使用电脑相关软件与被校准仪器的电信号接口连接好，通过软件读取仪器的压力数据。（通常使用串口小帮手这款通用软件、仪器专用的软件，或者相互配合使用），进行初始调试，建立ts文件。

### 压力实验：

- 连接仪器（有的需要通电、有的需要终端），有的需要压力罐。
- 分别将数据拷贝到0-1-2三个txt文件（2个0点，1个升降压过程）里（也有仪器直接生成cap或 cnv文件）。加减砝码的操作。 
- 填写当日的原始记录、实验室环境记录、设备使用记录等。
- 将当日电子版和纸质版原始记录拿回来。分别拷贝到专用电脑，纸质版复印多份，原始纸质版存档。
...
## LOG
2018-03-13 深夜创建本文件。
20180815 增加后面的部分，后面两个横线之间的内容等待与前面的部分进行整合。
